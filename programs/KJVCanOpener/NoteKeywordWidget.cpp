...
bool CNoteKeywordModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
	...
	if ((index.row() >= 0) && (index.row() < m_lstKeywordData.size())) {
		switch (role) {
			...
			case Qt::DisplayRole:
			{
				QString strDecompNewKeyword = StringParse::decompose(value.toString(), false);
				QString strDecompOldKeyword = StringParse::decompose(m_lstKeywordData.at(index.row()).m_strKeyword, false);
...
{
	QString strDecomposedKeyword = StringParse::decompose(strKeyword, false);
	int ndxFound = -1;
...
{
	return (StringParse::decompose(s1.first.m_strKeyword, false).compare(StringParse::decompose(s2.first.m_strKeyword, false), Qt::CaseInsensitive) < 0);
}

static bool decendingLessThan(const QPair<CNoteKeywordModelItemData, int> &s1, const QPair<CNoteKeywordModelItemData, int> &s2)
{
	return (StringParse::decompose(s2.first.m_strKeyword, false).compare(StringParse::decompose(s1.first.m_strKeyword, false), Qt::CaseInsensitive) < 0);
}
