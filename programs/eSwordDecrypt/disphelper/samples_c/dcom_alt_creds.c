...
HRESULT InitIdentity(LPWSTR szUserName, LPWSTR szPassword, LPWSTR szDomain)
{
	...
	if ((AuthIdentity.User = szUserName) != NULL)
		AuthIdentity.UserLength = wcslen(szUserName);

	if ((AuthIdentity.Password = szPassword) != NULL)
		AuthIdentity.PasswordLength = wcslen(szPassword);

	if ((AuthIdentity.Domain = szDomain) != NULL)
...
int main(void)
{
	...
	HR_TRY( InitIdentity(L"user_name", L"password", L"domain_or_machine_name") );
	HR_TRY( InitSecurity() );

	HR_TRY( CreateObjectWithAltCredentials(L"Word.Document", L"machine_name", &wdDoc) );

	HR_TRY( dhGetValue(L"%o", &wdApp, wdDoc, L".Application") );
